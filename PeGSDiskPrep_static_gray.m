...
RlargeC = 140; %What radius (in pixels) do we expect for the large discs?
...
%     photo_BW = im2double(photo_red_ch); % 转换为双精度浮点数，范围 [0, 1]，适合后续处理
%     Rimg = imadjust(photo_BW);  % 亮度调整 Adjust the top and bottom 1% intensities, for better circle finding
% 
%     % Black out bright areas near the rotor, to avoid particle detection there
%     rotor_xs = [Width/2-550 Width/2+100 Width/2+750];
%     rotor_ys = [Height Height-500 Height];
%     rotor_blank = roipoly(Rimg, rotor_xs, rotor_ys);
% 
%     Rimg = double(~rotor_blank).*Rimg;
...
% %     particle(p_no(i)).z = particle(p_no(i)).z - 1;  %Reduce the coordination number
        ...
        % 全通道处理
    photo = img;  
    photo_gray = rgb2gray(photo);  % 将彩色图像转换为灰度图像
    photo_BWall = im2double(photo_gray);  % 将灰度图像转换为双精度浮点数
    New_img = imadjust(photo_BWall);  % 亮度调整 Adjust the top and bottom 1% intensities, for better circle finding

        % Black out bright areas near the rotor, to avoid particle detection there
    rotor_xs = [Width/2-550 Width/2+100 Width/2+750];
    rotor_ys = [Height Height-500 Height];
    rotor_blank = roipoly(New_img, rotor_xs, rotor_ys);
    New_img = double(~rotor_blank).*New_img;
    ...
    I_corr_photo = polyval(coefficients_corr, dist_centre); % y = polyval(p,x) evaluates the polynomial p at each point in x
    ...
    % Eliminate bright area near the rotor
    photo_corr_2 = photo_corr(1:Height, 1:Width);
    rotor_xs = [Width/2-550 Width/2+100 Width/2+750];
    rotor_ys = [Height Height-500 Height];
    rotor_blank = roipoly(photo_corr_2, rotor_xs, rotor_ys);
    photo_corr_2 = double(~rotor_blank).*photo_corr_2;
...
%         imshow(Rimg);
          imshow(New_img);    %灰度图 %%%%%%%%%%%%%%%%%改%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        figure(2); %Draw the Force Image
        imshow(photo_for_pseudo); % 减弱圆盘边界，保留力链图像
    end
    
    if doParticleDetectionH %这个检测为什么用的是亮度修正之前的图而不是之后的
%         particle = PeGSDiskFindH(Rimg, RlargeH, SL, RsmallH, SS, pxPerMeter, fsigma);
        particle = PeGSDiskFindH(New_img, RlargeH, SL, RsmallH, SS, pxPerMeter, fsigma);  %%%%%%%%%%%%%%%%%%改%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% Flexible selection for the contactG2Threshold value 阈值选择好详细
...
%% Neighbour finding and plotting the results
    ...
    % Uncomment below to remove contacts manually 手动删除错误的连接
    %%  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%改%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    %%
    % 这里的ID是什么,不是图片上标注出的数字
%     p_no = [1,2，98]; %对白光报错 IDs of particles with false contacts
    p_no = [];  % 去掉正常了
    % 红白光的颗粒标记编号变了
    c_no = [1,1,4];  % 去掉上面的下面居然会正常运行就很神奇Which contact is erroneous
    for i = 1:length(p_no)
    particle(p_no(i)).z = particle(p_no(i)).z - 1;  %Reduce the coordination number
...
%%%%%%%%%%%%%%%%%%%% 可视化力网络 %%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for n = 1:N
        ...
        %如果一个颗粒有两个峰值并且其力小于2.0，那么将其峰值数设置为1，这一步还没有用到peaks，后面solver会用到它
